...
3. **Stage 7 prompt rewrite**: timing stated as fixed input ("this scene is 172 frames; line 1 visible 0.4–3.1s"). Post-process overwrites any LLM-emitted `durationInFrames`/`beats` with computed values. — **done (#75)**; repair patches are re-timed the same way.
