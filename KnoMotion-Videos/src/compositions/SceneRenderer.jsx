...
export const SceneTransitionWrapper = ({
  ...
  const enterProgress = clamp01(frame / transitionFrames);
  const exitProgress = clamp01(
    (durationInFrames - frame) / transitionFrames,
  );
  ...
  if (settings.type === 'slide') {
    ...
    const enterOffset = (1 - enterProgress) * 1;
    const exitOffset = (1 - exitProgress) * -1;
